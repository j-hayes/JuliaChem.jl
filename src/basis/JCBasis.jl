...
function run(molecule, model; output="none")
  ...
  auxillary_basis::String = haskey(model, "auxillary_basis") ? model["auxillary_basis"] : ""
  ...
  if !isempty(auxillary_basis)
    auxillary_basis_set_shells = Vector{JCModules.Shell}([])
    auxillaryshells_cxx = StdVector([ StdVector{JERI.Shell}() for i in 1:55 ]) 
    auxillaryshells_cxx_added = [ false for i in 1:55 ]  
  end
  ...
  h5open(joinpath(@__DIR__, "../../records/bsed.h5"),"r") do bsed
    ...
    for (atom_idx, symbol) in enumerate(symbols)
      ...
      shell_id, basis_set_norb, pos = add_shell!(shells, shells_cxx, shell_am_mapping, basis_set_shells,
       shell_id, atom_idx, atomic_number, atom_center, pos, shells_cxx_added, basis_set_norb, output)
...
function add_shell!(shells, shells_cxx, shell_am_mapping, basis_set_shells,
    shell_id, atom_idx, atomic_number, 
    atom_center, pos, shells_cxx_added, basis_set_norb, output)
  ...
  return shell_id, basis_set_norb, pos
end
