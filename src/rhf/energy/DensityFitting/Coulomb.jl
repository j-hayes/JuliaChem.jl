...
function calculate_dfrhf_coulomb_no_sym!(scf_data::SCFData, scf_options::SCFOptions) 
    ...
    for Q_range_index in 1:size(scf_data.B, 1)
        BLAS.gemv!('N', one, scf_data.B[Q_range_index], scf_data.density_array, zero, scf_data.V_batches[Q_range_index])
        BLAS.gemv!('T', two, scf_data.B[Q_range_index], scf_data.V_batches[Q_range_index], zero, scf_data.J[Q_range_index])
