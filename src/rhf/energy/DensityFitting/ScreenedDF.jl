...
function df_rhf_fock_build_screened!(scf_data, jeri_engine_thread_df::Vector{T}, jeri_engine_thread::Vector{T2},
    ...
    if iteration == 1
        ...
        #open an hdf5 file for two_center_integrals
        h5file = h5open("two_center_integrals-gly8.h5", "w")
        write(h5file, "two_center_integrals", two_center_integrals)
        ...
        #write J_PQ 
        write(h5file, "J_PQ", two_center_integrals)
        close(h5file)
        B_time = 0.0
        three_eri_time = 0.0
        if n_ranks > 1 
        ...
        scf_data.V_batches = Vector{Array{Float64,3}}(undef, num_Q_ranges)
        ...
        for ii in 1:num_Q_ranges
            ...
            scf_data.V_batches[ii] = zeros(Float64, length(Q_range))
        end
    ...
    if iteration == 1
        h5file = h5open("coulomb-gly8.h5", "w")
        write(h5file, "J_pq_screened", scf_data.J)
        write(h5file, "J", scf_data.two_electron_fock)
        write(h5file, "V", scf_data.V_batches[1])
        close(h5file)
    ...
    if iteration == 1
        h5file = h5open("fock-gly8.h5", "w")
        write(h5file, "fock", scf_data.two_electron_fock)
        close(h5file)
    end
end
...
function calculate_exchange_block_screen_matrix(scf_data, scf_options, default_n_blocks, jc_timing::JCTiming)
    ...
    # n_non_square_blocks = min(lower_triangle_length % n_threads, scf_data.μ % K_block_width)
